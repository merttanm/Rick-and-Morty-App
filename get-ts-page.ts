...
function getJSONExample(viewModel) {
    // >> get-json-code-ts
    getJSON("https://rickandmortyapi.com/api/character/1").then((r: any) => {
        // >> (hide)
        viewModel.set("host", r.name);
        viewModel.set("userAgent", r.status);
        viewModel.set("origin", r.species);
        viewModel.set("url", r.gender);
        // << (hide)
    }, (e) => {
        // >> (hide)
        console.log("Error: ");
